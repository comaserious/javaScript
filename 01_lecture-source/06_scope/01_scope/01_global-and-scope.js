...
/*
    ...
    젼역변수는 어디서든지 참조할 수 있다.
    지역이란 함수 몸체(코드블럭)내부를 말하며, 지역은 지역 스코프를 만든다
...
// 유효한 스코프 밖이기 때문에 출력할수가 없다
// console.log(z);   

/*
    스코프 체인
    전역 스코프는(x, y, outer() 함수 <= outer() 의 지역 스코프는 z, inner() <- inner 지역 스코프는 지역변수 x )
    모든 스코프는 하나의 계층적 구조로 연결되며, 모든 지역 스코프의 최상위 스코프는 전역 스코프이다
    변수를 참조할 때 자바 스크립트 엔진은 스코프 체인을 통해 변수를 참조하는 코드의 스코프에서 시작하여 
